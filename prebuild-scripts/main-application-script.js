...
const projectDir = path.join(rootDir, 'android', 'app', 'src', 'main', 'java');

// Recursive function to search for MainApplication.java
...
// Find the MainApplication.java file
const mainApplicationFilePath = findMainApplication(projectDir);

if (!mainApplicationFilePath) {
  console.error('MainApplication.java file not found.');
  return;
}

// Read the existing content of MainApplication.java
fs.readFile(mainApplicationFilePath, 'utf8', (err, fileContent) => {
  ...
  if (fileContent.includes(importStatement)) {
    console.log('Import statement already exists in MainApplication.java.');
  } else {
  ...
  if (fileContent.includes(lineToAdd)) {
    console.log('Line already exists in MainApplication.java.');
  } else {
  ...
  if (fileContent.includes(statementToAdd)) {
    console.log('Statement already exists in MainApplication.java.');
  } else {
  ...
  fs.writeFile(mainApplicationFilePath, fileContent, 'utf8', (err) => {
    ...
    console.log('Modifications applied to MainApplication.java file.');
  });
});
