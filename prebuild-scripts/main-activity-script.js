...
if (!mainActivityFilePath) {
  console.error('MainActivity.java file not found.');
  return;
}

// Read the existing content of MainActivity.java
fs.readFile(mainActivityFilePath, 'utf8', (err, fileContent) => {
  ...
  if (fileContent.includes(importStatement)) {
    console.log('Import statement already exists in MainActivity.java.');
  } else {
  ...
  if (fileContent.includes('shouldAuthenticate()')) {
    console.log('shouldAuthenticate() method already exists in MainActivity.java.');
  } else {
  ...
  fs.writeFile(mainActivityFilePath, fileContent, 'utf8', (err) => {
    ...
    console.log('Modifications applied to MainActivity.java file.');
  });
});
